...
(function ($) {
	var DrSlider = function (element, options) {
		...
		this.options = {
			...
			navigationColor: '#e4e4e4',
			navigationHoverColor: '#cecece',
			navigationHighlightColor: '#7bbedf',
	...
	DrSlider.prototype = {
		...
		_attachNavigation: function () {
			...
			for (var i = 0; i < this.num_slides; i++)
				navigation_html += '<span class="nav-link index' + i + '" style="margin-right:10px;display:inline-block" data-num="' + i + '"></span>';
			...
			if (!this.options.userCSS) {
				...
				$nav_link.css({
					...
					'width': this.options.navigationType == 'number' ? '50px' : '13px',
	...
	$.fn.DrSlider = function (options) {
		...
		return this.each(function () {
			...
			var data = $this.data('DrSlider');
			if (!data) {
				data = new DrSlider(this, options);
				$this.data('DrSlider', data);
